...
import { AlertTriangle, CheckCircle, Heart, FileDown, ArrowDownToLine } from "lucide-react";
...
const ResultsPanel = ({ results, imageUrl }: ResultsPanelProps) => {
    ...
    try {
        ...
        <body>
          ...
          <div class="section">
            ...
            <div class="status ${abnormal ? 'abnormal' : 'normal'}">
              ${abnormal ? 'Potential Abnormalities Detected' : 'No Significant Abnormalities'}
            </div>
      ...
      if (conditions.length > 0) {
        ...
        conditions.forEach(condition => {
          const colorClass = condition.probability > 70 ? 'high' : condition.probability > 50 ? 'medium' : '';
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "There was an error generating the report.",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className={`border-l-4 ${abnormal ? 'border-l-amber-500' : 'border-l-green-500'}`}>
          ...
          <CardTitle className="flex items-center gap-2">
            {abnormal ? (
              ...
              <>
                ...
                <span>No Significant Abnormalities</span>
              </>
            )}
          </CardTitle>
          <CardDescription>
                ...
                {conditions.map((condition, index) => (
                  <li key={index} className="space-y-1">
                    <div className="flex justify-between items-center">
                      ...
                      <Badge 
                        variant={condition.probability > 70 ? "destructive" : "outline"}
                        className={condition.probability > 70 ? "" : "text-amber-600 dark:text-amber-400"}
                      >
                        {condition.probability}% probability
                      </Badge>
                    </div>
                    <Progress value={condition.probability} className={condition.probability > 70 ? "bg-red-100" : ""} />
                  </li>
                ))}
              </ul>
            ) : (
      ...
      <div className="text-xs text-gray-500 mt-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-md">
        <strong>Disclaimer:</strong> This analysis is provided for educational purposes only and is not a substitute for professional medical advice. 
