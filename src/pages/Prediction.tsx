...
// );
    ...
    doc.text(`Report Generated: ${results.timestamp}`, pageWidth - margin, 15, { align: "right" });
    doc.text(`Report ID: VSR-${Date.now().toString().slice(-8)}`, pageWidth - margin, 20, { align: "right" });
    ...
    doc.setFillColor(heartRateColor[0], heartRateColor[1], heartRateColor[2], 0.1);
    ...
    const heartRateLines = doc.splitTextToSize(results.heartRate.message, pageWidth - margin * 2 - 85);
    ...
    doc.text("Reference Range: Normal adult heart rate is typically 60-95 BPM at rest.", margin, yPos);
    ...
    // Split long message into multiple lines if needed
    const spo2Lines = doc.splitTextToSize(results.spo2.message, pageWidth - margin * 2 - 85);
    ...
    doc.setFont("helvetica", "italic");
    doc.setTextColor(lightTextColor[0], lightTextColor[1], lightTextColor[2]);
    doc.text("Reference Range: Normal SpO2 is typically ≥ 95% in healthy adults.", margin, yPos);
    ...
    doc.roundedRect(margin, yPos, pageWidth - 2 * margin, summaryBlockHeight, 3, 3, 'FD');
    
    // Urgency Level
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(urgencyColor[0], urgencyColor[1], urgencyColor[2]);
    doc.text(`Assessment: ${results.summary.urgencyLevel.toUpperCase()} SITUATION`, margin + 5, yPos + 12);
  ...
  return (
    <MainLayout>
      <div className="max-w-3xl mx-auto p-6 bg-white rounded-xl shadow-lg border border-blue-100 mt-8">
        <h1 className="text-3xl font-extrabold text-blue-800 mb-6 text-center tracking-tight">
          Vital Signs Prediction
        </h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div>
            <label htmlFor="heartRate" className="block text-sm font-semibold text-gray-700 mb-2">
              Heart Rate (BPM)
            </label>
            <input
              id="heartRate"
            ...
            />
            <p className="mt-1 text-xs text-gray-500">Normal range: 60 - 95 BPM</p>
          </div>
          <div>
            <label htmlFor="spo2" className="block text-sm font-semibold text-gray-700 mb-2">
              SpO₂ (%)
            </label>
            <input
              id="spo2"
        ...
        <div className="text-center mb-8">
          <button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded shadow transition"
          >
            Evaluate
          </button>
        </div>
        {results && (
          <div className="space-y-6">
            {/* Heart Rate Result */}
            <div className={`${getStatusColor(results.heartRate.status)} p-4 rounded-lg flex items-start`}>
              <div className="mr-4">{getHeartRateIcon(results.heartRate.status)}</div>
              <div>
                <h2 className="text-xl font-bold text-gray-800">
                  Heart Rate: {heartRate} BPM - <span className="capitalize">{results.heartRate.status}</span>
                </h2>
                <p className="mt-2 text-gray-700">{results.heartRate.message}</p>
              </div>
            </div>
            {/* SpO2 Result */}
            <div className={`${getStatusColor(results.spo2.status)} p-4 rounded-lg flex items-start`}>
              <div className="mr-4">{getOxygenIcon(results.spo2.status)}</div>
              <div>
                <h2 className="text-xl font-bold text-gray-800">
                  SpO₂: {spo2}% - <span className="capitalize">{results.spo2.status}</span>
                </h2>
            ...
            <div className="p-4 rounded-lg bg-blue-50 border border-blue-300">
              ...
              <p className={`font-bold ${getUrgencyColor(results.summary.urgencyLevel)}`}>
