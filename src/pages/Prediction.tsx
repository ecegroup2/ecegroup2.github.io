...
export default function VitalSignsMonitor() {
  ...
  useEffect(() => {
    ...
    // Clean up function
    return () => {
      const scriptElement = document.querySelector('script[src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"]');
      if (scriptElement) {
  ...
  const checkSpO2 = (o2: number) => {
    if (o2 >= 95) {
      ...
      return {
        ...
        message: "SpO2 between 80% and 85% indicates hypoxemia that requires medical attention."
      };
    } else {
      return {
        status: "Critical Hypoxemia",
        message: "SpO2 below 80% indicates severe hypoxemia requiring supplemental oxygen immediately."
      };
    }
  };
  
  const generateSummary = (heartRateResult: { status: string; message: string }, spo2Result: { status: string; message: string }) => {
    ...
    if (spo2Result.status === "Severe Hypoxemia" || spo2Result.status === "Critical Hypoxemia") {
      urgencyLevel = "Urgent";
    ...
    if (heartRateResult.status === "Tachycardia") {
      recommendations.push("Monitor for symptoms such as shortness of breath or chest pain");
    }
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      doc.text('Vital Signs Diagnostic Report', pageWidth/2, 20, { align: 'center' });
      
      // Add timestamp
      doc.setFontSize(smallFont);
      doc.setFont('helvetica', 'italic');
      doc.setTextColor(100, 100, 100);
      doc.text(`Generated on: ${results.timestamp}`, pageWidth/2, 30, { align: 'center' });
      ...
      doc.text(`${results.summary.urgencyLevel.toUpperCase()} SITUATION`, 20, 140);
      ...
      doc.text('This report is generated by Vital Signs Diagnostic System.', pageWidth/2, yPos, { align: 'center' });
      doc.text('For medical emergencies, call 911 or your local emergency services immediately.', pageWidth/2, yPos + 5, { align: 'center' });
      doc.text('This report is not a substitute for professional medical advice, diagnosis, or treatment.', pageWidth/2, yPos + 10, { align: 'center' });
      
      // Save the PDF
      doc.save(`vital-signs-report-${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (err) {
  ...
  const getStatusRGBColor = (status: string): [number, number, number] => {
    switch(status) {
      case "Normal": return [22, 163, 74]; // green
      case "Bradycardia":
      case "Tachycardia":
      case "Mild Hypoxemia": return [202, 138, 4]; // yellow
      case "Moderate Hypoxemia": return [234, 88, 12]; // orange
      case "Severe Hypoxemia":
      case "Critical Hypoxemia": return [220, 38, 38]; // red
      default: return [75, 85, 99]; // gray
    }
  };
  
  // Helper function to get RGB color based on urgency level
  const getUrgencyRGBColor = (urgencyLevel: string): [number, number, number] => {
    switch(urgencyLevel) {
      case "Normal": return [22, 163, 74]; // green
      case "Caution": return [202, 138, 4]; // yellow
      case "Urgent": return [220, 38, 38]; // red
      default: return [75, 85, 99]; // gray
    }
  };
  ...
  const getStatusClass = (status: string): string => {
    ...
    if (status === "Bradycardia" || status === "Tachycardia" || status === "Mild Hypoxemia") return "caution";
  ...
  const getStatusColor = (status: string): string => {
    switch(status) {
      ...
      case "Bradycardia":
      case "Tachycardia":
      case "Mild Hypoxemia": return "bg-yellow-100 text-yellow-800 border-yellow-300";
      case "Moderate Hypoxemia": return "bg-orange-100 text-orange-800 border-orange-300";
      case "Severe Hypoxemia":
      case "Critical Hypoxemia": return "bg-red-100 text-red-800 border-red-300";
  ...
  return (
    ...
    <div className="max-w-[34rem] mx-auto mt-7 p-6 bg-gradient-to-b from-blue-50 to-white rounded-xl shadow-lg">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-blue-800">Vital Signs Monitor</h1>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-white p-4 rounded-lg shadow-md border border-blue-100">
          <label className="block text-sm font-medium text-blue-800 mb-2">Heart Rate (BPM)</label>
        ...
        <div className="bg-white p-4 rounded-lg shadow-md border border-blue-100">
          <label className="block text-sm font-medium text-blue-800 mb-2">SpO2 (%)</label>
          <div className="relative">
            <input 
              type="number" 
              value={spo2 ?? ''} 
      ...
      <button 
        ...
        className={`w-full py-3 px-4 text-white font-semibold rounded-lg shadow-md transition duration-300 flex items-center justify-center
          ${error ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-blue-600 to-blue-800 hover:from-blue-700 hover:to-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2'}`}
      >
        ...
        <div className="mt-8 bg-white p-5 rounded-lg shadow-lg border border-blue-100">
          ...
          <div className="mb-5 flex items-start">
            ...
            <div className="flex-1">
              <h3 className="font-medium text-blue-800">Heart Rate Assessment:</h3>
              <div className={`inline-block px-3 py-1 text-sm rounded-full mt-1 border ${getStatusColor(results.heartRate.status)}`}>
                {results.heartRate.status} - {heartRate} BPM
              </div>
              <p className="text-sm mt-2 text-gray-700">{results.heartRate.message}</p>
            </div>
          </div>
          ...
          <div className="mb-5 flex items-start">
            ...
            <div className="flex-1">
              ...
              <div className={`inline-block px-3 py-1 text-sm rounded-full mt-1 border ${getStatusColor(results.spo2.status)}`}>
                {results.spo2.status} - {spo2}%
              </div>
              <p className="text-sm mt-2 text-gray-700">{results.spo2.message}</p>
            </div>
          </div>
          
          <div className="mt-6 bg-blue-50 p-4 rounded-lg border border-blue-200">
            <h3 className="font-bold text-blue-800 mb-2">Summary Assessment:</h3>
            <div className={`text-lg font-bold mb-2 ${getUrgencyColor(results.summary.urgencyLevel)}`}>
      ...
      <div className="mt-8 p-4 bg-blue-50 rounded-lg border border-blue-100 text-xs text-gray-600">
        ...
        This tool is for informational purposes only and is not a substitute for professional medical advice or diagnosis. Always consult a healthcare provider for medical concerns. Do not use in emergencies.
        </p>
     </div>
    </div>
    </MainLayout>
